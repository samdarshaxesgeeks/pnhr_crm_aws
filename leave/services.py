...
from leave.models.leave_record import LeaveRecord
...
def create_leave_record(employee, year):
    try:
        previous_leave_record = LeaveRecord.objects.get(employee=employee, leave_year=year - 1)
    except LeaveRecord.DoesNotExist:
        previous_leave_record = None

    residue = 0
    if previous_leave_record is not None:
        residue = previous_leave_record.balance
